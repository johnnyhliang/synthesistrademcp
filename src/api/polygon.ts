...
export async function getOrders(client: SynthesisClient): Promise<OrderRecord[]> {
  client.requireAuth();
  return client.get<OrderRecord[]>('polygon/orders');
}

export async function getSwaps(client: SynthesisClient): Promise<SwapRecord[]> {
  client.requireAuth();
