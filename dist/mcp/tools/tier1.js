 ...
 */
import { z } from 'zod';
import { listMarkets, searchMarkets, getSparklines, getMarketPrices, getOrderbooks, getHistoricalOrderbooks, getStatistics, getRelatedMarkets, getSimilarMarkets, getSimilarPairs, } from '../../api/markets.js';
...
import { listKalshiMarkets, getKalshiMarket, getKalshiEvent, getKalshiMarketBySlug, getKalshiTrades, getKalshiHolders, getKalshiStatistics, getKalshiPriceHistory, getKalshiCandlesticks, getKalshiLeaderboard, getKalshiUser, } from '../../api/kalshi.js';
...
export function registerTier1Tools(server, client) {
    ...
    // ── get_market_prices ───────────────────────────────────────────────────────
    server.tool('get_market_prices', 'Get current prices for multiple markets in one request. Pass Polymarket token IDs or Kalshi market IDs (up to 5000).', {
    ...
    // ── get_orderbooks ──────────────────────────────────────────────────────────
    server.tool('get_orderbooks', 'Get current orderbooks for multiple markets in one request (up to 5000).', {
        markets: z.array(z.string()).min(1).max(5000).describe('Array of Polymarket token IDs or Kalshi market IDs'),
    }, async (params) => {
    ...
    // ── get_sparklines ──────────────────────────────────────────────────────────
    server.tool('get_sparklines', 'Get historical price sparklines for a batch of markets.', {
    ...
    server.tool('get_polymarket_market_by_slug', 'Get a Polymarket event by slug.', {
        slug: z.string(),
    ...
    server.tool('get_polymarket_price_history', 'Get historical prices and OHLC candles for a Polymarket token.', {
    ...
    server.tool('get_polymarket_trades', 'Get recent trades for a Polymarket market.', {
    ...
    server.tool('get_polymarket_statistics', 'Get aggregate statistics (change, high, low) for a Polymarket token.', { token_id: z.string() }, async (params) => {
    ...
    server.tool('get_kalshi_market', 'Get a specific Kalshi market by market ID (e.g. KXPRESNOMD-28-GN).', { market_id: z.string() }, async (params) => {
        const market = await getKalshiMarket(client, params.market_id);
        return { content: [{ type: 'text', text: JSON.stringify(market, null, 2) }] };
    });
    server.tool('get_kalshi_event', 'Get a Kalshi event and all its markets by event ID.', {
        event_id: z.string(),
        sort: z.string().optional(),
        order: z.enum(['ASC', 'DESC']).optional(),
    }, async (params) => {
    ...
    server.tool('get_kalshi_market_by_slug', 'Get a Kalshi event by slug.', {
        slug: z.string(),
        sort: z.string().optional(),
        order: z.enum(['ASC', 'DESC']).optional(),
    }, async (params) => {
    ...
    server.tool('get_kalshi_trades', 'Get recent trades for a Kalshi market.', {
        market_id: z.string(),
        limit: z.number().int().max(1000).optional().describe('Default 100'),
        offset: z.number().int().optional(),
    }, async (params) => {
    ...
    server.tool('get_kalshi_holders', 'Get token holder distribution for a Kalshi market.', { market_id: z.string() }, async (params) => {
        const holders = await getKalshiHolders(client, params.market_id);
        return { content: [{ type: 'text', text: JSON.stringify(holders, null, 2) }] };
    });
    server.tool('get_kalshi_statistics', 'Get aggregate statistics (change, high, low) for a Kalshi market.', { market_id: z.string() }, async (params) => {
        const stats = await getKalshiStatistics(client, params.market_id);
        return { content: [{ type: 'text', text: JSON.stringify(stats, null, 2) }] };
    });
    server.tool('get_kalshi_price_history', 'Get historical price data for a Kalshi market with optional time bucketing.', {
        ...
        const data = await getKalshiPriceHistory(client, series_id, kalshi_id, rest);
        return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
    });
    server.tool('get_kalshi_candlesticks', 'Get OHLCV candlestick data for a Kalshi market.', {
        series_id: z.string(),
        kalshi_id: z.string(),
        ...
        const { series_id, kalshi_id, ...rest } = params;
        const data = await getKalshiCandlesticks(client, series_id, kalshi_id, rest);
        return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
    });
    server.tool('get_kalshi_leaderboard', 'Get the Kalshi platform leaderboard.', {
    ...
    server.tool('get_kalshi_user', 'Get public profile and trading metrics for a Kalshi username.', { username: z.string() }, async (params) => {
    ...
    server.tool('get_event_news', 'Get news articles related to a specific prediction market event.', {
        event_id: z.string(),
        limit: z.number().int().optional(),
        offset: z.number().int().optional(),
    }, async (params) => {
    ...
    server.tool('get_market_news', 'Get news articles related to a specific market.', {
        market_id: z.string(),
        limit: z.number().int().optional(),
        offset: z.number().int().optional(),
    }, async (params) => {
